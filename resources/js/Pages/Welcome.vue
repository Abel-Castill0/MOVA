<template>
  ...
  <div class="min-h-screen bg-white">
      ...
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20">
        <div class="grid lg:grid-cols-2 gap-16 items-center">
          <div class="text-white">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur rounded-full text-sm font-medium mb-6 border border-white/20">
            ...
            <p class="text-xl text-white/80 leading-relaxed mb-10 max-w-lg">
              Conectamos estudiantes con profesores particulares expertos para clases en vivo por Zoom. Personalizado, flexible y efectivo.
            </p>
          ...
          <!-- 3D Card visual -->
          <div class="hidden lg:flex justify-center items-center perspective-1000">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div v-for="(step, i) in steps" :key="i"
            class="group relative bg-white rounded-2xl p-8 border border-gray-100 hover:border-brand-200 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2"
            v-scroll-reveal>
    ...
    <!-- ── Materias ────────────────────────────────────────────────────── -->
    <section id="materias" class="py-24 bg-slate-50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          <Link v-for="s in subjects" :key="s.id"
            ...
            class="group bg-white border border-gray-100 hover:border-brand-300 rounded-xl p-4 text-center hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
            ...
            <span :class="['text-xs px-2 py-0.5 rounded-full mt-1 inline-block capitalize', levelColor(s.level)]">{{ s.level }}</span>
          </Link>
        </div>
        <div class="text-center mt-10">
          <Link :href="route('marketplace')" class="inline-flex items-center gap-2 px-6 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition-colors shadow-lg shadow-brand-600/20">
    ...
    <section id="profesores" class="py-24 bg-white">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
        ...
        <div v-if="featuredTeachers.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="t in featuredTeachers" :key="t.id"
            class="group bg-white rounded-2xl border border-gray-100 hover:border-brand-200 p-6 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-start gap-4 mb-4">
              <div class="w-14 h-14 bg-gradient-to-br from-brand-500 to-brand-700 rounded-full flex items-center justify-center text-white font-black text-xl flex-shrink-0">
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-slate-400 mt-0.5">{{ t.classes_count }} clases impartidas</p>
              </div>
              <span class="ml-auto text-xs px-2 py-1 bg-green-50 text-green-700 rounded-full font-medium border border-green-100 flex-shrink-0">✓ Verificado</span>
            </div>
            <p class="text-sm text-slate-500 mb-4 line-clamp-2">{{ t.bio || 'Profesor experto en MOVA.' }}</p>
            <div class="flex flex-wrap gap-1.5 mb-4">
              <span v-for="s in (t.subjects || []).slice(0, 3)" :key="s.id"
                class="text-xs bg-brand-50 text-brand-700 px-2.5 py-1 rounded-full font-medium border border-brand-100">
                {{ s.name }}
              </span>
            </div>
        ...
        <div v-else class="text-center py-16">
          <p class="text-slate-400">No hay profesores verificados aún. <Link :href="route('landing.teacher')" class="text-brand-600 hover:underline">¿Eres profesor? Únete</Link></p>
        </div>
      </div>
    </section>

    <!-- ── Testimonios ────────────────────────────────────────────────── -->
    <section class="py-24 bg-gradient-to-br from-brand-900 to-brand-800 relative overflow-hidden">
          ...
          <div v-for="t in testimonials" :key="t.name"
            ...
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
                :style="{ background: t.color }">{{ t.name.charAt(0) }}</div>
              <div>
      ...
      <div class="max-w-3xl mx-auto px-4 text-center">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <Link :href="route('register')"
            class="px-8 py-4 bg-brand-600 text-white font-bold rounded-xl hover:bg-brand-700 transition-all shadow-xl shadow-brand-600/25 hover:scale-105">
            Crear cuenta gratis
          </Link>
          <Link :href="route('marketplace')"
            class="px-8 py-4 bg-brand-50 text-brand-700 font-bold rounded-xl hover:bg-brand-100 transition-all border border-brand-200">
            Explorar profesores
          </Link>
        </div>
      </div>
...
const steps = [
  ...
  { icon: '🎓', title: 'Aprende en vivo', desc: 'Conéctate por Zoom y aprende de manera personalizada, con ejercicios y seguimiento en tiempo real.' },
]

const testimonials = [
  { name: 'María López', role: 'Madre de familia', quote: '"Gracias a MOVA, mi hijo aprobó el examen de matemáticas que tenía pendiente. El profesor fue increíblemente paciente y claro."', color: 'linear-gradient(135deg,#667eea,#764ba2)' },
  { name: 'Carlos Méndez', role: 'Estudiante universitario', quote: '"Encontré a un profesor de Cálculo que explica de forma que por fin entiendo. Las clases son 100% online y muy cómodas."', color: 'linear-gradient(135deg,#f093fb,#f5576c)' },
  { name: 'Laura Sánchez', role: 'Profesora de inglés', quote: '"Unirme como profesora fue muy fácil. Ahora tengo alumnos fijos y organizo mis horarios como yo quiero."', color: 'linear-gradient(135deg,#4facfe,#00f2fe)' },
]
...
function levelColor(level) {
  return { primaria: 'bg-green-50 text-green-700', secundaria: 'bg-blue-50 text-blue-700', universidad: 'bg-purple-50 text-purple-700', todos: 'bg-slate-100 text-slate-600' }[level] ?? 'bg-gray-50 text-gray-600'
}
