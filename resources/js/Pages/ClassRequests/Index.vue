<template>
  <AppLayout title="Mis solicitudes">
    <div class="space-y-6">
      ...
      <div v-if="!requests.length" class="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <p class="font-semibold text-slate-900">{{ r.subject?.name }}</p>
              ...
              <div v-if="r.status === 'teacher_rejected' && r.teacher_rejection_reason"
                class="mt-2 text-xs text-red-600 bg-red-50 rounded-lg px-2 py-1">
            ...
            <div v-if="r.status === 'pending_parent_approval'" class="flex gap-2">
              <Link :href="route('class-requests.approve', r.id)" method="post" as="button"
                class="px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-xl hover:bg-green-700 active:scale-95 transition-all">
                Aprobar
              </Link>
              <Link :href="route('class-requests.reject', r.id)" method="post" as="button"
                class="px-3 py-1.5 bg-red-100 text-red-700 text-xs font-bold rounded-xl hover:bg-red-200 active:scale-95 transition-all">
                Rechazar
              </Link>
            </div>
          </div>
