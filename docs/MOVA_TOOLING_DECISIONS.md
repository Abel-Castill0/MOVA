...
| Cláusula de retiro de contenido subido por usuarios (para que la responsabilidad legal recaiga en quien sube, no en MOVA) | ✅ `Terms.vue` §12, "Política de Retiro de Contenido" — cita el Decreto Legislativo 822 (Ley de Derecho de Autor), traspasa la responsabilidad al usuario, y define un proceso de solicitud de retiro |
...
| Firebase | MOVA usa Laravel + MySQL como backend/BD — Firebase sería una base de datos/backend paralela sin ningún encaje con la arquitectura actual. |
| Supabase | Mismo caso — MOVA no usa Postgres/Supabase, usa MySQL gestionado en Railway. |
| Polar.sh | MOVA ya está construyendo Culqi como pasarela de pago (fundación ya commiteada esta sesión) — añadir una segunda pasarela sin necesidad de negocio no aporta nada hoy. |
| React Native skills | MOVA es una app web (Inertia), no tiene ni planea una app móvil nativa. |
| n8n / n8n-mcp | Requiere un servidor n8n propio corriendo en algún lado — no existe ninguno provisionado para MOVA. Sin ese servidor, el MCP no tiene a qué conectarse. Si en el futuro decides automatizar algo (recordatorios, RPA), evalúo esto de nuevo con la necesidad concreta primero. |
...
### Seguridad
La lista de 20 ítems que pegaste (rate limiting, RLS, env vars, validación, auth, etc.) ya fue el objeto de la auditoría F-01 a N1 de esta misma sesión — Policies, `lockForUpdate()`, throttle en cada mutación, `ProviderGuard` fail-closed, HMAC en webhooks, etc. La única brecha real de esa lista que sigue abierta es la de secretos en git (F-26/F-27, ya documentada, pendiente de tu rotación).
