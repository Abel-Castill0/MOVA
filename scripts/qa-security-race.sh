...
case "$OPERATION" in
  ...
  complaints-*)
    UNIQUE="$(grep -o 'code=MOVA-[0-9-]*' <<<"$OUT" | sort -u | wc -l)"
    YEAR="$(date +%Y)"
    ...
    echo "unique_codes=$UNIQUE rows_this_year=$ROWS expected_rows=$EXPECTED_ROWS"
    { [ "$ACCEPTED" -eq "$WORKERS" ] && [ "$UNIQUE" -eq "$WORKERS" ] && [ "$ERRORS" -eq 0 ] && [ "$ROWS" -eq "$EXPECTED_ROWS" ]; } || STATUS=1 ;;
esac
